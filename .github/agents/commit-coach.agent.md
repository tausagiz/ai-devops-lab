---
name: "Commit Coach"
description: "Use to validate current changes, apply docs-gate staging fixes, and create a compliant commit. Prefer reusing recent @Validate Changes scope context when available."
...
## Workflow
1. **Check branch**: If current branch is `main` or `master`, stop immediately. These branches are protected; commits must go through feature branches and pull requests. Suggest `@Branch Coach` to start a feature branch first.
...
11. Infer next workflow step from context:
    - If user plan mentions further work, changes, or branch continuation -> suggest `@Branch Coach` to start the next task.
    - If commit closes active work on this branch (user intent is clear) -> suggest `@PR Coach`.
    - If unclear -> default to `@PR Coach` and add a one-line note: "If you plan to continue with more work on a new branch, run `@Branch Coach` instead."
12. On successful commit, include one short friendly sentence that explains how to run the next command in Copilot Chat using an `@Agent Name` entrypoint.
...
- **Never commit on `main` or `master`**: always stop and suggest `@Branch Coach` first.
- Commit only when invoked via `@Commit Coach` or when user explicitly asks to commit.
...
3. **Usage hint** (required on successful commit): one short sentence that says the next command is a Copilot Chat entrypoint typed in chat (`@Agent Name` or `/Command`). Add that other tools should run the equivalent workflow command.
4. **Next Step** (always required; never omit or replace with a clarifying question):
   - On successful commit, choose `@PR Coach` or `@Branch Coach` based on inferred context (see step 10 of the workflow). Output a `### Next Step` markdown heading followed by a fenced block containing only the command:
...
- When context suggests continuing with more work, offer `@Branch Coach` instead of `@PR Coach`.
- When blocked on main, skip scope-drift check and offer only `@Branch Coach` as next step.
