---
name: "Check Scope"
description: "Check whether this branch drifted beyond its intended scope and suggest the smallest corrective action."
...
### Next Action
- One short action. Copilot example: if `continue-with-rescope`, type `/Rescope Branch` in chat and then `@Commit Coach`; if `split-now`, type `@Split Scope`. Other tools: run the equivalent branch split workflow.
