...
- Educational sandbox for AI-assisted learning workflows and course experiments.
...
- `learning/` - course index and course-specific content.
- `.github/agents/` and `.github/prompts/` - thin tool-specific wrappers for workflow entrypoints.
- `.github/instructions/` - concise coding/testing conventions for the repository.
- `tests/unit/` - unit tests for currently active implementations.
- `tests/integration/` - lightweight integration checks when relevant.
- `scripts/check_docs.py` - commit message + docs gate validation.

## Repository rules

- Implementation: keep learning modules isolated by topic and prefer small, reversible changes; avoid coupling course examples across domains unless explicitly needed.
...
- Branch-first execution guardrail: never start implementation edits on `main` or `master`. If user asks to implement and current branch is `main`/`master`, stop before any file edit and run branch-start flow first (`create branch for <task>`). Exception: explicit user instruction to work directly on `main`.
...
- Validation and PR: by default run `python scripts/check_docs.py`; run `pytest tests/unit` and `pytest tests/integration` only when `test_*.py` files exist in those directories; narrower scope only on explicit request and then clearly state what was skipped; full validation only on explicit request (`pytest tests/ --cov --cov-report=term-missing`); target PR to `main` by default; if branch is behind `main`, ask for `merge` or `rebase` unless preference is already explicit; before push, stop on dirty worktree and ask the user to commit the changes first (for example: `commit these changes`); include extra PR context in the body.
